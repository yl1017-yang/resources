(function () {
  const slideBox = document.querySelector('.slide_box_ver3');
  const slideList = document.querySelector('.slide_list3');
  const slideContents = document.querySelectorAll('.slide_content3');
  const slideBtnNext = document.querySelector('.slide_btn_next3');
  const slideBtnPrev = document.querySelector('.slide_btn_prev3');
  const slideBtnPlay = document.querySelector('.slide_btn_play3');
  const slideBtnStop = document.querySelector('.slide_btn_stop3');
  const pagination = document.querySelector('.slide_pagination3');
  ...
  let pageChild = '';
  for (var i = 0; i < slideLen; i++) {
    pageChild += '<li class="dot dot3';
    pageChild += (i === curIndex) ? ' dot_active3' : '';
    pageChild += '" data-index="' + i + '"><a href="#"></a></li>';
  }
  pagination.innerHTML = pageChild;
  const pageDots = document.querySelectorAll('.dot3');
  ...
  function nextMove() {
    ...
    pageDots[(curIndex === -1) ? slideLen - 1 : curIndex].classList.remove('dot_active3');
    curSlide = slideContents[++curIndex];
    curSlide.classList.add('slide_active');
    pageDots[curIndex].classList.add('dot_active3');
  }
  ...
  function prevMove() {
    ...
    curSlide.classList.remove('slide_active');
    pageDots[(curIndex === slideLen) ? 0 : curIndex].classList.remove('dot_active3');
    curSlide = slideContents[--curIndex];
    curSlide.classList.add('slide_active');
    pageDots[curIndex].classList.add('dot_active3');
  }
  ...
  Array.prototype.forEach.call(pageDots, function (dot, i) {
    dot.addEventListener('click', function (e) {
      e.preventDefault();
      curDot = document.querySelector('.dot_active3');
      curDot.classList.remove('dot_active3');
      curDot = this;
