...
function checkShot() {  
  ...
  for (let i = 0; i < arrNumber.length; i++) {
    ...
    .then(canvas => {
      rowsaveImg(canvas.toDataURL("image/jpeg", 0.8), ("1000X1000_" + arrNumber[i] + "_" + name + ".jpg"));
    });
  }
}
...
function partShot(download_index) {
  ...
  setTimeout(function () {
    ...
    .then(canvas => {
      ...
      saveImg(canvas.toDataURL("image/jpeg", 0.7), wantedWidth, wantedHeight, (wantedWidth + "X" + wantedHeight + "_" + arrNumber[download_index] + "_" + name + ".jpg"));
